  ...
  return (
    <div className="vcf-container py-6 pb-24 space-y-8 font-sans">
      ...
      <div className="space-y-4 pb-6 border-b border-hairline">
        <div className="flex items-center gap-2">
          <span className="bg-brand-primary text-white font-mono text-xs px-2.5 py-0.5 font-semibold rounded-full">
            MÃ: {program.code}
          </span>
        ...
        <p className="text-base text-ink-secondary leading-relaxed font-sans max-w-3xl">
          {program.shortDesc}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
        ...
        <div className="lg:col-span-8 space-y-10">
          ...
          <div className="space-y-4">
            ...
            <div className="p-5 bg-parchment border border-hairline rounded-lg text-xs">
              <strong className="text-ink block mb-1.5 uppercase font-semibold">Đối tượng tuyển sinh:</strong>
              <p className="text-ink-secondary leading-relaxed font-sans text-xs">{program.targetAudience}</p>
            </div>

            <div className="space-y-2 pt-2">
              <strong className="text-xs uppercase text-ink block font-semibold">Sau khóa học, học viên sẽ làm chủ:</strong>
          ...
          <div className="border-t border-hairline pt-8 space-y-4">
            ...
            <div className="space-y-4">
              {program.modules.map((mod, idx) => (
                <div key={idx} className="border border-hairline p-5 bg-white rounded-lg shadow-xs space-y-2">
                  <div className="flex items-center justify-between">
                    <h4 className="font-semibold text-sm text-ink">{mod.title}</h4>
                    <span className="text-[11px] font-semibold text-brand-primary bg-red-50 px-2.5 py-0.5 rounded-full border border-red-200">
                      {mod.duration}
                    </span>
                  </div>
                  <ul className="pl-4 list-disc text-xs text-ink-secondary space-y-1">
          ...
          <div className="border-t border-hairline pt-8 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {program.faculty.map((fac, idx) => (
                <div key={idx} className="border border-hairline p-4 bg-white rounded-lg shadow-xs flex items-center gap-3.5">
                  <div className="w-14 h-14 rounded-full bg-red-50 border border-red-100 flex items-center justify-center font-semibold text-xs text-brand-primary shrink-0">
                    GV
                  </div>
                  <div className="space-y-0.5">
                    <div className="font-semibold text-sm text-ink">{fac.name}</div>
                    <div className="text-xs text-ink-secondary font-medium">{fac.role}</div>
            ...
            <div className="p-5 border border-hairline bg-parchment rounded-lg space-y-3 text-xs">
              ...
              {program.scholarshipInfo && (
                <div className="pt-2 border-t border-hairline text-ink-secondary">
                  <strong className="text-ink">Ưu đãi: </strong> {program.scholarshipInfo}
                </div>
              )}
            </div>
          </div>
          ...
          <div className="border-t border-hairline pt-8 space-y-4">
            ...
            <div className="space-y-3">
              {program.upcomingSchedules.map((sch, idx) => (
                <div key={idx} className="border border-hairline p-4 bg-white rounded-lg shadow-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs">
                  <div>
                    <strong className="text-sm text-ink block">{sch.cohort}</strong>
                    <div className="text-ink-secondary">Khai giảng: {sch.startDate} • {sch.location}</div>
                  </div>
                  <span className="bg-red-50 text-brand-primary border border-red-200 px-3 py-1 font-semibold rounded-full">
                    {sch.status}
                  </span>
                </div>
              ))}
        ...
        <div className="lg:col-span-4 sticky top-24 space-y-6">
          ...
          <div className="border border-hairline bg-white p-5 rounded-lg space-y-3 shadow-xs">
            ...
            <div>
              ...
              {program.scholarshipInfo && (
                <div className="mt-2 text-xs bg-red-50 border border-red-200 text-brand-primary p-2.5 rounded-md font-medium">
                  {program.scholarshipInfo}
                </div>
              )}
            </div>
          ...
          <div className="border border-hairline bg-white p-6 md:p-7 rounded-lg space-y-5 shadow-sm">
            <div className="pb-3 border-b border-neutral-100">
              <span className="text-[10px] font-semibold uppercase bg-red-50 text-brand-primary border border-red-200 px-2.5 py-0.5 rounded-full">
                FORM LEAD TƯ VẤN
              </span>
            ...
            {leadStatus === 'success' ? (
              <div className="p-6 bg-red-50/30 border border-red-200 rounded-lg text-center space-y-3">
                <div className="w-10 h-10 bg-brand-primary text-white rounded-full mx-auto flex items-center justify-center shadow-xs">
              ...
              <form onSubmit={handleLeadSubmit} className="space-y-4 text-xs">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full p-2.5 border border-neutral-300 rounded-lg focus:outline-none focus:border-brand-primary font-sans"
                  />
                  {formErrors.fullName && <span className="text-[11px] text-brand-primary">{formErrors.fullName}</span>}
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full p-2.5 border border-neutral-300 rounded-lg focus:outline-none focus:border-brand-primary font-sans"
                  />
                  {formErrors.phone && <span className="text-[11px] text-brand-primary">{formErrors.phone}</span>}
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full p-2.5 border border-neutral-300 rounded-lg focus:outline-none focus:border-brand-primary font-sans"
                  />
                  {formErrors.email && <span className="text-[11px] text-brand-primary">{formErrors.email}</span>}
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <select
                    ...
                    className="w-full p-2.5 border border-neutral-300 rounded-lg bg-white font-sans focus:outline-none focus:border-brand-primary"
                  >
