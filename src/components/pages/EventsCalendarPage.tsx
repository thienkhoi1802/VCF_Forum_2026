...
export const EventsCalendarPage: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 pb-24 space-y-8 font-sans">
      ...
      {/* Header & Controls Bar */}
      <div className="flex flex-col md:flex-row md:items-end justify-between pb-6 border-b border-hairline gap-4">
        ...
        <div className="flex items-center gap-3">
          <div className="flex border border-hairline bg-neutral-100 p-1 rounded-full shadow-2xs">
            <button
              onClick={() => setViewMode('list')}
              className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-full transition-all duration-150 ${
                viewMode === 'list' ? 'bg-white text-brand-primary shadow-xs' : 'text-ink-secondary hover:text-ink'
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-full transition-all duration-150 ${
                viewMode === 'calendar' ? 'bg-white text-brand-primary shadow-xs' : 'text-ink-secondary hover:text-ink'
              }`}
            >
      ...
      <div className="relative z-20 bg-white border border-hairline rounded-xl p-2 sm:px-3.5 sm:py-2 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-2xs">
        ...
        <div className="flex items-center gap-2 self-end sm:self-auto">
          ...
          {selectedActivityFilter !== 'all' && (
            <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-red-50 text-brand-primary text-xs font-semibold border border-red-200 animate-in fade-in">
              ...
              <button
                ...
                className="hover:bg-red-100 rounded-full p-0.5 cursor-pointer text-brand-primary"
          ...
          <div className="relative" ref={filterDropdownRef}>
            <button
              ...
              className={`flex items-center gap-2 px-3 py-1.5 text-xs font-semibold rounded-lg border transition-all cursor-pointer ${
                selectedActivityFilter !== 'all'
                  ? 'bg-red-50 text-brand-primary border-red-200 shadow-2xs'
                  : 'bg-white text-neutral-700 border-hairline hover:bg-parchment hover:border-neutral-300'
              }`}
            >
            ...
            {isGeneralFilterOpen && (
              <div className="absolute right-0 top-full mt-2 w-72 bg-white border border-hairline rounded-xl shadow-xl p-3 z-50 animate-in fade-in zoom-in-95 duration-150">
                  ...
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-2.5 py-2 rounded-lg text-xs font-semibold transition-colors text-left cursor-pointer ${
                      selectedActivityFilter === 'all'
                        ...
                        : 'text-neutral-700 hover:bg-parchment'
                    }`}
                  >
                    <div className="flex items-center gap-2">
                      {selectedActivityFilter === 'all' && <Check className="w-3.5 h-3.5 text-brand-primary" />}
                      <span className={selectedActivityFilter === 'all' ? 'font-semibold' : ''}>Tất cả hoạt động</span>
                    </div>
                    <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-neutral-100 text-ink-secondary">
                      {MOCK_EVENTS.length}
                    </span>
                  </button>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2 truncate">
                          ...
                          <span className={`truncate ${isSelected ? 'font-semibold' : ''}`}>{act.title}</span>
                        </div>
                        <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-neutral-100 text-ink-secondary shrink-0 ml-2">
                          {count}
                        </span>
                      </button>
                    );
          ...
          {(timingFilter !== 'all' || selectedActivityFilter !== 'all') && (
            <button
              ...
              className="p-1.5 text-neutral-400 hover:text-brand-primary hover:bg-neutral-100 rounded-lg transition-colors cursor-pointer"
      ...
      {isLoading ? (
        ...
        <div className="border border-hairline bg-white rounded-lg p-12 text-center space-y-3 shadow-xs">
        ...
        <div className="border border-hairline bg-white rounded-lg p-6 space-y-6 shadow-xs">
          <div className="flex items-center justify-between border-b border-neutral-100 pb-4">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="p-2 border border-hairline rounded-full hover:bg-parchment text-ink-secondary"
              >
          ...
          {/* Calendar Grid */}
          <div className="grid grid-cols-7 gap-px bg-neutral-200 border border-hairline rounded-lg overflow-hidden text-xs">
            {['T2', 'T3', 'T4', 'T5', 'T6', 'T7', 'CN'].map((day, i) => (
              <div key={i} className="bg-parchment p-2.5 text-center font-semibold text-neutral-700">
                {day}
              </div>
            ))}
              ...
              return (
                <div
                  ...
                  className={`bg-white min-h-[95px] p-2 flex flex-col justify-between transition-colors ${
                    eventsOnDay.length > 0 ? 'bg-red-50/40 border border-red-200 font-semibold' : 'hover:bg-parchment'
                  }`}
                >
                    ...
                    <button
                      ...
                      className={`text-[10px] text-left p-1 rounded-sm truncate mt-1 block shadow-2xs cursor-pointer ${
                        ev.status === 'past'
                          ...
                          : 'bg-brand-primary text-white hover:bg-[#c80e00]'
                      }`}
                      title={ev.title}
                    >
                ...
                <React.Fragment key={evt.id}>
                    ...
                    <div className="pt-6">
                      <div className="flex items-center justify-between pb-2 border-b-2 border-neutral-300">
                        <div className="flex items-center gap-2">
                          ...
                          <h2 className="text-lg font-semibold text-neutral-800 uppercase tracking-tight">
                            Sự Kiện Đã Diễn Ra ({pastEvents.length})
                          </h2>
                          <span className="text-xs bg-neutral-100 text-ink-secondary font-semibold px-2 py-0.5 rounded border border-neutral-300">
                            Kho lưu trữ & Kỷ yếu
                          </span>
                        </div>
                        <span className="text-xs text-ink-secondary hidden sm:inline font-medium">
                  ...
                  >
                    ...
                    <div className="flex md:hidden items-center justify-between pb-3 border-b border-neutral-100">
                      <div className="flex items-center gap-2">
                        <div className={`w-2 h-2 rounded-full ${isPast ? 'bg-neutral-400' : 'bg-brand-primary'}`} />
                        <span className={`text-xs font-semibold uppercase tracking-wider ${isPast ? 'text-ink-secondary' : 'text-brand-primary'}`}>
                      ...
                      <div className="text-[11px] font-mono font-medium text-ink-secondary bg-neutral-100 px-2 py-0.5 rounded border border-hairline flex items-center gap-1">
                    ...
                    <div className="hidden md:flex flex-col items-center justify-center w-24 lg:w-28 shrink-0 text-center py-2 pr-5 border-r border-neutral-100 select-none">
                      <span className={`text-[11px] font-semibold uppercase tracking-wider block ${isPast ? 'text-ink-secondary' : 'text-brand-primary'}`}>
                        {parseDateBlock(evt.dateStr).monthText}
                      </span>
                      <span className="text-3xl lg:text-4xl font-semibold text-neutral-900 block leading-none my-1 tracking-tight">
                        {parseDateBlock(evt.dateStr).day}
                      </span>
                      <span className="text-xs font-semibold text-neutral-400 block">
                        {parseDateBlock(evt.dateStr).year}
                      </span>
                      <div className="mt-3 inline-flex items-center gap-1 text-[11px] font-mono font-medium text-ink-secondary bg-parchment px-2 py-1 rounded-md border border-hairline">
                    ...
                    <div className="w-full md:w-64 lg:w-72 shrink-0 rounded-xl overflow-hidden bg-neutral-100 self-start">
                      <WireframeImage
                        ...
                        aspectRatio="16:9"
                        className="w-full h-full min-h-[150px] md:min-h-[175px] object-cover rounded-xl border border-hairline group-hover:scale-105 transition-transform duration-300"
                      />
                    </div>
                    ...
                    <div className="flex-1 flex flex-col justify-between space-y-3.5">
                      <div className="space-y-3">
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          <span className={`text-[11px] font-semibold uppercase px-2.5 py-0.5 rounded-full border ${
                            isPast
                          ...
                          {isPast ? (
                            <span className="text-[11px] font-semibold text-ink-secondary bg-neutral-100 border border-neutral-300 px-2.5 py-0.5 rounded-full">
                              ĐÃ DIỄN RA
                            </span>
                          ) : (
                            <span className="text-[11px] font-semibold text-emerald-800 bg-emerald-50 border border-emerald-300 px-2.5 py-0.5 rounded-full flex items-center gap-1.5">
                          ...
                          {!isPast && (
                            <>
                              {isFullEffective ? (
                                <span className="text-[11px] font-semibold text-rose-800 bg-rose-100 border border-rose-300 px-2.5 py-0.5 rounded-full flex items-center gap-1.5 shadow-2xs">
                                  <span className="w-1.5 h-1.5 rounded-full bg-rose-600 shrink-0" />
                                  Hết chỗ
                                </span>
                              ) : (
                                <span className="text-[11px] font-semibold text-emerald-800 bg-emerald-100/90 border border-emerald-300 px-2.5 py-0.5 rounded-full flex items-center gap-1.5 shadow-2xs">
                          ...
                          {regItem?.status === 'pending_approval' && (
                            <span className="text-[11px] font-semibold text-amber-800 bg-amber-50 border border-amber-300 px-2.5 py-0.5 rounded-full flex items-center gap-1.5 shadow-2xs">
                              <span className="w-2 h-2 rounded-full bg-amber-500 animate-pulse" />
                              Đang chờ duyệt
                            </span>
                          )}
                          {regItem?.status === 'waitlisted' && (
                            <span className="text-[11px] font-semibold text-amber-800 bg-amber-50 border border-amber-300 px-2.5 py-0.5 rounded-full flex items-center gap-1">
                              Danh sách chờ
                            </span>
                          )}
                        </div>
                        ...
                        {/* 2. Title */}
                        <h3 className={`text-lg sm:text-xl font-semibold leading-snug transition-colors ${
                          isPast
                        ...
                        <div className="bg-parchment/80 border border-hairline/80 rounded-xl p-3 flex items-start gap-2.5 text-xs">
                          <div className="w-6 h-6 rounded-md bg-red-50 border border-red-100 flex items-center justify-center shrink-0 mt-0.5">
                            <MapPin className={`w-3.5 h-3.5 ${isPast ? 'text-ink-secondary' : 'text-brand-primary'}`} />
                          </div>
                          <div>
                            <span className="text-[10px] font-semibold uppercase text-neutral-400 block leading-tight">Địa điểm</span>
                            <span className="font-semibold text-neutral-900 leading-snug block" title={evt.location}>
                              {evt.location}
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="text-xs text-ink-secondary flex items-center gap-1.5">
                          {evt.speakers.length > 0 && (
                            <>
                              ...
                              <strong className="text-ink font-semibold">{evt.speakers[0].name}</strong>
