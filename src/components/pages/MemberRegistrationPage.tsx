...
export const MemberRegistrationPage: React.FC = () => {
  ...
  const handleEmailSubmit = (e: React.FormEvent) => {
    ...
    void registerWithEmail({ email, password, fullName }).then(result => {
      if (!result.ok) {
        setSignupState('idle');
        setErrors(prev => ({ ...prev, email: result.error || 'Không thể tạo tài khoản.' }));
        showNotification(result.error || 'Không thể tạo tài khoản.');
        return;
      }

      if (result.requiresEmailConfirmation) {
        setSignupState('email-verify-sent');
        showNotification(`Liên kết xác thực đã được gửi tới email ${email}.`);
      } else {
        setSignupState('success');
        showNotification('Tạo tài khoản thành công! Chào mừng bạn gia nhập Diễn đàn CEO Việt Nam.');
      }
    });
  };

  const handleConfirmEmail = () => {
    void refreshAuthSession().then(isAuthenticated => {
      if (isAuthenticated) {
        setSignupState('success');
        showNotification('Email đã được xác thực. Tài khoản của bạn đã sẵn sàng.');
      } else {
        showNotification('Chưa thấy phiên đăng nhập. Vui lòng bấm liên kết trong email rồi thử lại.');
      }
    });
  };

  return (
    <div className="max-w-2xl mx-auto px-6 lg:px-8 py-8 pb-24 space-y-8 font-sans">
      {showSpecAnnotations && (
      ...
      {signupState === 'success' ? (
        ...
        <div className="bg-white border border-hairline rounded-2xl p-6 sm:p-8 text-center space-y-6 shadow-sm animate-fadeIn max-w-lg mx-auto">
          ...
          <div className="space-y-4 text-left">
            <div className="p-4 bg-parchment border border-hairline rounded-xl text-sm text-ink-secondary leading-relaxed">
              Hãy mở email xác thực của Supabase và bấm vào liên kết bên trong để kích hoạt tài khoản. Sau khi xác thực, bạn có thể quay lại đây để kiểm tra phiên đăng nhập.
            </div>

            <button
              type="button"
              onClick={handleConfirmEmail}
              className="w-full py-3.5 bg-brand-primary hover:bg-brand-primary-hover text-white rounded-xl font-semibold text-sm uppercase tracking-wide transition-colors shadow-md cursor-pointer"
            >
            ...
            <button
              ...
              className="w-full text-center hover:underline text-ink-secondary text-xs cursor-pointer"
            >
